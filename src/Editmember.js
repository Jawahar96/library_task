...
function Editmember() {
  ...
  const formik=useFormik({
   ...
   onSubmit :async (values)=>{
    let member = await axios.put(`https://642a6fd600dfa3b54745b1cd.mockapi.io/members/${params.id}`,values)
 ...
 let loadMember =async (values)=>{
    try{
        let member = await axios.get(`https://642a6fd600dfa3b54745b1cd.mockapi.io/members/${params.id}`)
        formik.setValues({
            
            name:member.data.name,
            cardid:member.data.cardid,
            Authorname:member.data.Authorname,
